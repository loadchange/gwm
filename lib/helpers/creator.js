...
export const observer = (target, container, callback) => {
  ...
  const containerObserver = new MutationObserver(mutationsList => mutationsList.forEach(mutation => mutation.removedNodes.forEach(_target => _target === target && callback())));
